...
void SMITH(char *x, int m, char *y, int n, int verbose) {
    ...
    while (j <= n - m) {
        ...
        j += MAX(bmBc[(unsigned char)y[j + m - 1]], qsBc[(unsigned char)y[j + m]]);
    }
}
int main(int argc, char *argv[]) {
    ...
    for (int r = 0; r < repeat; r++) {
        run_once(SMITH, pattern, plen, text, tlen, &time_ms, &comps, verbose);
        printf("%f,%llu\n", time_ms, comps);
    }

...
static void preZtBc(char *x, int m, int ztBc[ASIZE][ASIZE]) {
    int i, j;
    for (i = 0; i < ASIZE; ++i)
        for (j = 0; j < ASIZE; ++j)
            ztBc[i][j] = m;
...
void ZT(char *x, int m, char *y, int n, int verbose) {
    ...
    while (j <= n - m) {
        ...
        if (i < 0) {
            ...
            int shift = bmGs[i];
            int ztShift = ztBc[(unsigned char)y[j + m - 2]][(unsigned char)y[j + m - 1]];
            if (ztShift > shift) shift = ztShift;
            j += shift;
        }
    }
}
int main(int argc, char *argv[]) {
    ...
    for (int r = 0; r < repeat; r++) {
        run_once(ZT, pattern, plen, text, tlen, &time_ms, &comps, verbose);
        printf("%f,%llu\n", time_ms, comps);
    }
